...
class EKFSLAM:
    ...
    def add_landmarks(
        ...
        for j in range(numLmk):
            ...
            Gx[inds, 2] = np.array([zj[0] @ np.array([-np.sin(zj[1] + eta[2]),
                                                      np.cos(zj[1] + eta[2])]).T + sensor_offset_world_der @ self.sensor_offset])# TODO
    ...
    def update(
        ...
        if numLmk > 0:
            ...
            if za.shape[0] == 0:
                etaupd, Pupd = self.predict(eta,P,z) # TODO
